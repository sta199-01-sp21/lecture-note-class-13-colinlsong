...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-colinlsong/manhattan.csv")
```
...
In repeated samples, 95% of these samples will contain the true mean. 
...
We are 95% confident that the true median of price of 1-bedroom apartments in 
Manhattan falls within this range of 2162 and 2875 dollars.
...
We are 95% confident that the true proportion that true proportion of mice that 
died while not on treatment lies within the range 0.69 and 0.797. 
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop1, level = .90)
ci_mice_lived
```
...
visualise(boot_prop1) +
  shade_ci(ci_mice_lived)
```


Give an interpretation of your result.
We are 90% confident that the true proportion of mice that lived while on the 
treatment lies somewhere between 0.552 and 0.651. 
